...
//		elem.addOrUpdateTag(createdByTag, createdByKey);
	...
	private URL getCapabilitiesUrl() throws MalformedURLException {
		...
		int apiPos = getReadWriteUrl().indexOf(SERVER_API_PATH);
		if (apiPos > 0) {
	...
	/**
	 * Return the url as a string for a read only API if it exists otherwiese the result is the same as for read/write
	 * @return
	 */
